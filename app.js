...
function renderResource(resource, step) {
  ...
  const isSearchLesson = /written guide search|practice search|dream project search|parts list search/i.test(resource.type || '') ||
    (resource.trustLabel === 'Suggested Search' && /google\.com\/search/i.test(resource.url || ''));
  const isWritten = /written|guide|documentation|docs|course|article|textbook/i.test(resource.type || '') && !isVideoSearch;
  const summary = (isWritten || isSearchLesson) ? getResourceSummary(resource, step, state.userProfile) : null;
  const resourceUrl = safeUrl(resource.url);
  return `
    <article class="resource-card ${isSearchLesson ? 'search-lesson-card' : ''}">
      ...
      <div class="resource-body">
        <div class="resource-labels"><span>${esc(isSearchLesson ? 'in-product lesson' : resource.type || 'resource')}</span><span class="trust">${esc(isSearchLesson ? 'Summarized by CircuitMentor' : resource.trustLabel || 'Curated')}</span><span>${esc(resource.board || step.boardUsed)}</span><span>${esc(resource.difficulty || step.difficulty)}</span></div>
        <h4>${esc(isSearchLesson ? resource.title.replace(/search/ig, 'lesson') : resource.title)}</h4>
        ...
        ${isSearchLesson ? '' : `<div class="resource-actions">
          <a class="text-link ${embed || resource.trustLabel !== 'Suggested Search' ? 'primary-link' : ''}" href="${esc(resourceUrl)}" target="_blank" rel="noopener noreferrer">${embed ? 'Open direct video on YouTube' : isWritten ? 'Open full guide' : 'Open resource'} ↗</a>
        </div>`}
        ${isSearchLesson && summary ? `<div class="search-lesson">
          <span class="search-lesson-kicker">COMPLETE BEGINNER SUMMARY · NO WEB SEARCH NEEDED</span>
          <p><b>What you need:</b> ${esc(summary.whatToRead)}</p>
          <p><b>Ignore for now:</b> ${esc(summary.whatToIgnoreForNow)}</p>
          <p><b>In plain language:</b> ${esc(summary.beginnerSummary)} ${esc(step.whyItMatters)}</p>
          <p><b>What you are learning:</b> ${esc(step.knowledgeToLearn)}</p>
          <p><b>How to use it:</b></p>
          <ol>${summary.keySteps.map(item => `<li>${esc(item)}</li>`).join('')}</ol>
          <p><b>Common mistake:</b> ${esc(summary.commonMistake)}</p>
          <p><b>Safety check:</b> ${esc(step.safetyNote)}</p>
          <div class="search-lesson-task"><b>Try it now</b><span>${esc(summary.miniTask)}</span></div>
        </div>` : summary ? `<details class="guide-summary"><summary>Beginner guide summary</summary><div>
          <p><b>What you need:</b> ${esc(summary.whatToRead)}</p>
          <p><b>Ignore for now:</b> ${esc(summary.whatToIgnoreForNow)}</p>
