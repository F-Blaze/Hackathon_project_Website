...
import { verifiedVideoMatches } from './lib/verified_video_matches.js';
...
export function generateRoadmap(profile, selectedPathType) {
  ...
  const steps = raw.map((step, index) => {
    const primaryResources = findResourcesForStep(step, profileForResources, 4);
    const hasUnresolvedVideoSearch = primaryResources.some(resource =>
      /video search/i.test(resource.type || '') && !verifiedVideoMatches[resource.id]
    );
    const candidates = hasUnresolvedVideoSearch
      ? [...primaryResources, ...findResourcesForStep(step, profileForResources, 12)]
      : primaryResources;
    const resources = [];
    const usedResources = new Set();
    for (const candidate of candidates) {
      const isVideoSearch = /video search/i.test(candidate.type || '');
      const resolved = isVideoSearch ? verifiedVideoMatches[candidate.id] : candidate;
      if (!resolved) continue;
      const key = `${resolved.title}|${resolved.url}`;
      if (usedResources.has(key)) continue;
